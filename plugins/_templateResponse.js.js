...
export async function all(m, chatUpdate) {
  ...
  const messageText = (id || text || '').trim();
  if (!messageText) return; // تجنب تنفيذ الأمر في حال عدم وجود نص
  ...
  const messages = await generateWAMessage(m.chat, { text: messageText, mentions: m.mentionedJid }, {
