...
async function fesnuk(postUrl, cookie = "", userAgent = "") {
    if (!postUrl || !postUrl.trim()) throw new Error("💮هات الرابط💮");
    if (!/(facebook.com|fb.watch)/.test(postUrl)) throw new Error("🌸الرابط غير متاح 🌸");
...
function parseString(string) {
    try {
        return JSON.parse(`{"💮العنوان": "${string}"}`).text;
    } catch (e) {
        return string;
    }
}
...
let handler = async (m, { args, conn }) => {
    ...
    if (sdQuality) {
        ...
        m.reply(`🌸العنوان: ${result.title}\nالجوده: SD`);
    } else {
        m.reply("Unable to fetch the video in SD quality.");
    }
};

handler.help = handler.command = ['فيس','فيسبوك'];
