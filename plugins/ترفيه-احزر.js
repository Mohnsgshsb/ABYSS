...
const poin = 500;
const game = '.احزر'; // متغير لتخزين اسم اللعبة
...
export async function handler(m, { command, text, conn }) {
    ...
    if (currentGame) {
        if (!text) {
            ...
            conn.sendButton(m.chat, `> *◞🎊◜ مـبـارك لـقـد ربـحـت حـصـلـت عـلـي: ${poin}ريـال💰*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍷ــزيـد‹◝`, `${game}`]], null, null);
            ...
            conn.sendButton(m.chat, `> *◞😂◜ تـم الانـسـحـاب بـنـجـاح ايـهـا الـفـاشـل الاجـابـة كـانـت: ${currentGame[1].name}*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍷ــزيـد‹◝`, `${game}`]], null, null);
            ...
            conn.sendButton(m.chat, `> *◞❌◜ الاجـابـة  خـاطـئـه الاجـابـه الـصـحـيحه كـانـت : ${currentGame[1].name}*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍷ــزيـد‹◝`, `${game}`]], null, null);
        ...
        if (!text) {
            ...
            conn.tebakGame[id] = [m, question, 10, setTimeout(() => {
                ...
                conn.sendButton(m.chat, `> *◞⏰  ◜انـتـهـي وقـت الـعـبـة الاجـابـة كـانـت: ${question.name}*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍷ــزيـد‹◝`, `${game}`]], null, null);
            }, gameDuration)];
...
`;
            ...
            m.react('👇🏻');
            conn.sendButton(m.chat, `> *لـقـد انـتـهـت الـعـبـة ◞❕◜*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍷ــزيـد‹`, `${game}`]], null, null);
        }
    }
}
