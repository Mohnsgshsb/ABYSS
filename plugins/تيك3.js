...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) {
    return conn.reply(m.chat, `❲ ❗ ❳ حدث خطأ عند البحث عن الرابط . يرجى إدخال رابط صحيح، مثال:\n\n➤ ${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/`, m);
  }
...
`.trim();
    ...
    if (type === "slide") {
      ...
      if (videoInfo.nowm) {
        ...
        conn.reply(m.chat, "> ❲ ❗ ❳ ➤ فشل في جلب الفيديو بدون علامة مائية.", m);
      }
    }
    ...
    console.error(error);
    conn.reply(m.chat, `❲ ❗ ❳ حدث خطأ عند البحث عن الرابط . يرجى إدخال رابط صحيح، مثال:\n\n➤ ${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/`, m);
  }
};

handler.help = ["tiktokdl"];
handler.tags = ["downloader"];
handler.command = ["تيك2"];
