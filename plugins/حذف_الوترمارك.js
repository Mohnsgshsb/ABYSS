...
let handler = async (m, { conn, text }) => {
  ...
  if (!imageUrl) return m.reply("💮 *| اعمل رييلاي علي الصوره عشان أشيل العلامه المائيه* ");

  try {
    await m.reply(" > ⏳ اصبر يشيل العلامه المائيه اهو");
    const remover = new TextWatermarkRemover();
    const resultUrl = await remover.removeTextWatermark({ imageUrl });
    await conn.sendFile(m.chat, resultUrl, "result.png", "✅ تم حذف العلامه المائية!", m);
  } catch (e) {
    m.reply("❌ Error: " + e.message);
  }
};

handler.help = handler.command = ["حذف_الوترمارك"];
handler.tags = ["tools"];
handler.limit = true;
