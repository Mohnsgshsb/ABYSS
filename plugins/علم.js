...
const poin = 500;
const game = '.علم'; // متغير لتخزين اسم اللعبة
...
export async function handler(m, { command, text, conn }) {
    ...
    let src = await (await fetch('https://raw.githubusercontent.com/SungReved/games/main/Src/علم.json')).json();
...
`;
            ...
            m.react('👇🏻');
            conn.sendButton(m.chat, `↬⌯⌛ انتهى الوقت. الإجابة الصحيحة هي: 『${name}』‹◝`, poster, [[`↬⌯الـمـ🍧ـزيـد‹◝`, `${game}`]], null, null);
        }
    }
}
