...
export async function handler(m, { command, text, conn }) {
    ...
    let src = await (await fetch('https://raw.githubusercontent.com/SungReved/games/main/Src/عين.json')).json();
    ...
    if (currentGame) {
        if (!text) {
            ...
            conn.sendButton(m.chat, `> *◞🎊◜ مـبـارك لـقـد ربـحـت حـصـلـت عـلـي: ${poin}ريـال💰*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍧ـزيـد‹◝`, `${game}`]], null, null);
            ...
            conn.sendButton(m.chat, `> *◞😂◜ تـم الانـسـحـاب بـنـجـاح ايـهـا الـفـاشـل الاجـابـة كـانـت: ${currentGame[1].name}*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍧ـزيـد‹◝`, `${game}`]], null, null);
            ...
            conn.sendButton(m.chat, `> *◞❌◜ الاجـابـة  خـاطـئـه الاجـابـه الـصـحـيحه كـانـت : ${currentGame[1].name}*`, `> ${botname}`, poster, [[`↬⌯الـمـ🍧ـزيـد‹◝`, `${game}`]], null, null);
...
`;
            ...
            m.react('👇🏻');
            conn.sendButton(m.chat, `↬⌯⌛ انتهى الوقت. الإجابة الصحيحة هي: 『${name}』‹◝`, `> ${botname}`, poster, [[`↬⌯الـمـ🍧ـزيـد‹◝`, `${game}`]], null, null);
        }
    }
}
